...
ssize_t TcpConnectionImpl::sendNodeInLoop(const BufferNodePtr &nodePtr)
{
    ...
    if (nodePtr->isFile() && !tlsProviderPtr_)
    {
        ...
        LOG_TRACE << "send file in loop using linux kernal sendFile()";
...
SSLContextPtr xiaoNet::newSSLContext(const TLSPolicy &policy, bool isServer)
{
    (void)policy;
    (void)isServer;
...
std::shared_ptr<TLSProvider> xiaoNet::newTLSProvider(TcpConnection *conn,
                                                     TLSPolicyPtr policy,
